...
class BoxEditorView(QtWidgets.QGraphicsView):
    def __init__(
        ...
        self.custom_scene: BoxEditorScene = BoxEditorScene(
            self, undo_stack, engine_manager, self.property_editor, self.project, None
        )
        ...
        if self.project:
            if self.project.header_y:
                self.scene().add_header_footer(
                    HEADER_FOOTER_ITEM_TYPE.HEADER, self.project.header_y
                )
            if self.project.footer_y:
                self.scene().add_header_footer(
                    HEADER_FOOTER_ITEM_TYPE.FOOTER, self.project.footer_y
                )
    ...
    def analyze_layout_cv(self, recognize=False) -> None:
        new_boxes: list[Box] = []
        scene = self.scene()
        if isinstance(scene, BoxEditorScene):
            if scene.image:
                image = self.pixmap_to_cv2(scene.image)
